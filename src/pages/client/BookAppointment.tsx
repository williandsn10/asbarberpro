...
export default function BookAppointment() {
  ...
  const [formData, setFormData] = useState({ service_id: "", appointment_date: "", appointment_time: "", notes: "" });
  ...
  const { data: barber } = useQuery({ queryKey: ["default-barber"], queryFn: async () => { const { data } = await supabase.from("barbers").select("*").eq("is_active", true).limit(1).maybeSingle(); return data; } });
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-6 animate-fade-in">
      ...
      {step === 2 && (
        <Card className="glass-card"><CardHeader><CardTitle className="flex items-center gap-2"><Calendar className="w-5 h-5 text-primary" />Escolha a Data e Horário</CardTitle></CardHeader>
          <CardContent className="space-y-4">
            ...
            <div className="flex gap-2"><Button variant="outline" onClick={() => setStep(1)}>Voltar</Button><Button onClick={() => setStep(3)} disabled={!formData.appointment_date || !formData.appointment_time} className="flex-1 bg-gradient-gold">Continuar</Button></div>
          </CardContent>
        </Card>
      )}
      ...
      {step === 3 && (
        <Card className="glass-card"><CardHeader><CardTitle>Confirmar Agendamento</CardTitle></CardHeader>
          <CardContent className="space-y-4">
            <div className="p-4 rounded-lg bg-secondary/50 space-y-2"><p><strong>Serviço:</strong> {selectedService?.name}</p><p><strong>Data:</strong> {formData.appointment_date.split("-").reverse().join("/")}</p><p><strong>Horário:</strong> {formData.appointment_time}</p><p className="text-xl font-bold text-primary">Total: R$ {selectedService?.price.toFixed(2)}</p></div>
