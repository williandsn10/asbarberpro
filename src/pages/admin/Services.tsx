...
export default function Services() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    selectedService ? updateMutation.mutate({
  ...
  return <div className="space-y-6 animate-fade-in">
      ...
      <Card className="glass-card">
        ...
        <CardContent>
          {isLoading ? <div className="flex justify-center py-8"><Loader2 className="w-6 h-6 animate-spin text-primary" /></div> : services.length === 0 ? <div className="text-center py-8 text-muted-foreground"><Scissors className="w-12 h-12 mx-auto mb-3 opacity-50" /><p>Nenhum serviço cadastrado</p></div> : <Table>
        ...
        <DialogContent className="glass-card">
          ...
          <form onSubmit={handleSubmit}>
            <div className="space-y-4 py-4">
              <div className="space-y-2"><Label>Nome</Label><Input value={formData.name} onChange={e => setFormData({
                ...formData,
                name: e.target.value
              })} required className="bg-secondary" /></div>
              <div className="space-y-2"><Label>Preço (R$)</Label><Input type="number" step="0.01" value={formData.price} onChange={e => setFormData({
                ...formData,
                price: e.target.value
              })} required className="bg-secondary" /></div>
              <div className="space-y-2"><Label>Duração (minutos)</Label><Input type="number" value={formData.duration_minutes} onChange={e => setFormData({
                ...formData,
